...
WIND_PROB_VALUES = {
    ...
    "0.05": 5, "0.15": 15, "0.30": 30, "0.45": 45,
    "0.60": 60, "0.75": 75, "0.90": 90,
}
HAIL_PROB_VALUES = {
...
def cat_label(key):
    if key and key in CAT_META: return CAT_META[key][0]
    return NO_RISK[0]

def cat_circle(key):
    if key and key in CAT_META: return CAT_META[key][1]
    return NO_RISK[1]

def cat_color(key):
    if key and key in CAT_META: return CAT_META[key][2]
    return NO_RISK[2]
...
def best_prob(feats, prob_map, layer_name=""):
    vals = []
    for f in feats:
        for field in ["dn", "DN", "valid", "PROB", "prob", "label", "LABEL"]:
            raw = f.get(field)
            if raw is None:
                continue
        ...
        for m in mds:
            md_inner += ('<tr style="border-bottom:1px solid #f0e8c8;">'
                         ...
                         a(m["url"], "Mesoscale Discussion #" + m["num"],
                           "color:#1a3a5c;text-decoration:none;") + "</td></tr>")
        md_inner += "</table>"
    else:
...
def main():
    ...
    subject  = ("Colletti Weather Office: Nado Nomad's Convective Compass | " +
                 date_str + " | National: " + cat_label(nat1_key) +
